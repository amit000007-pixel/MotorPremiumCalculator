...
rdate.addEventListener("change",function(){
  ...
  if(rsdate.value && rdate.value){
    ...
    vage.textContent=age.toFixed(2);
  }
...
ND.addEventListener("change",function(){
  ...
  if(ND.checked){
    ...
    if(age>2.6 && (vtype.value=='GCV4' || vtype.value=='PCV Bus'||vtype.value=='PCV School Bus'||vtype.value=='MISC' || vtype.value=='PCV Taxi'||vtype.value=='3GCV')){
...
function saveAsImage1() {
  const container = document.getElementById('container');
  const pname = document.getElementById('Pname');
  const regno = document.getElementById('regno');
  
  // FORCE checkbox states before capture
  const checkboxes = container.querySelectorAll('input[type="checkbox"]');
  checkboxes.forEach(cb => {
    if (cb.checked) {
      cb.setAttribute('checked', 'checked');  // Ensure attribute exists
    } else {
      cb.removeAttribute('checked');
    }
  });
  
  const filename = `${pname.value.replace(/[^a-zA-Z0-9]/g, '_')}_${regno.value.replace(/[^a-zA-Z0-9]/g, '_')}_Quote.jpg`;
  ...
  htmlToImage.toJpeg(container, {
    ...
    pixelRatio: 1.5,
    filter: filter,
    skipAutoScale: true  // Helps form elements
  })
  .then((dataUrl) => {
    // Clean up attributes after capture
